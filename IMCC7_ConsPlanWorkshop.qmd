...
*Notes: You can "show"/"hide" all code by clicking on the drop down box on the top right corner of the page. We have also added the option to copy the code in each code chunk for use in R.*
...
Growing demands for food, energy, and infrastructure put marine ecosystems and their services at risk. To protect marine biodiversity against threats from the accelerating use of the marine environment, international targets aim to expand the global protected area network to 30% of the oceans by 2030. In addition, United Nations member states recently adopted a treaty for the conservation and sustainable use of biodiversity in the High Seas, an ambitious goal considering \<1% of the High Seas is currently highly protected. Meeting these international targets requires tools and methods that facilitate informed decision-making to balance biodiversity conservation with demands of industries using the environment.
...
# Installations for hand-on conservation planning.
...
Alternatively, if the installation fails for some reason, on your machine, try one of the following options to install either the HiGHS solver or the CBC solver.
...
Today you are a conservation officer helping plan the Galapagos Marine Reserve. The Galapagos Islands and surrounding waters are recognized globally for their unique species and biodiversity, such as the endemic giant tortoises, Galapagos penguins and marine iguanas. In 1998, the Ecuadorian Government created a marine reserve, covering 138,000 km^2^ around the islands, which at the time, made it the second largest marine reserve in the world. However, many of the species inhabiting Galapagos, such as the blue footed booby (*Sula nebouxii*) and silky shark (*Carcharhinus falciformis*), use the open ocean, and range outside the borders of the reserve. Thanks to tracking studies carried out over the quarter century since the creation of the reserve, we know much more about the movements of many of the species in and around the reserve, and can begin to address the question of whether the current reserve design provides adequate protection for them. We also know more about the distribution of key ocean habitats in our region. Given that industrial and semi-industrial fishing pressure in Ecuador's waters outside the reserve increased dramatically at the turn of the century, and that in recent years, large distant-water fishing fleets have been reported in the high seas surrounding Ecuador's Exclusive Economic Zone, residents of Galapagos are concerned that the existing reserve may not provide sufficient protection to the ocean and wildlife that their livelihoods depend on. They are campaigning for an expansion of the reserve, and have asked us to identify key areas that should be included in their proposed new design. However, the fleets operating around the reserve are concerned that increasing the size of the reserve may substantially affect their livelihoods. In addition to information on key habitats and species movements, we have obtained spatially explicit catch data from the two main fleets - the industrial tuna purse seine fleet and the national longline fleet for large pelagic species.
...
First, we load the necessary `R` packages. A good `R` package to help install versions of `R` packages in CRAN is **`pacman`**. We are going to install and load packages throughout the course of this workshop, but a common best practice is to install and load all necessary packages to run each script at the top of the `R` script.
...
> **Raster data** is usually saved as a GeoTIFF (Geo Tagged Image File Formats; `.tiff`). Another file format that is good at compressing/storing large amounts of data is the netCDF (Network Common Data Form; `.nc`).\
...
If you already have a solver on your machine, comment these out and make sure that the solver is loaded. The solver is the mathematical algorithm that will solve the prioritization problem.
...
We now have all the necessary information needed to run the prioritization. Next, we set up the conservation "problem" using the `problem()` function in `prioritizr`. For this, we define all the spatial data (i.e., in this example, the object `out_sf`), what the features are called in `out_sf`, and the what the name of the cost column name in `out_sf`. We then use the minimum set objective function `add_min_set_objective()` to solve this conservation problem (i.e., minimizing the cost while meeting all of the features' targets). We also assign targets for each of the features using `add_relative_targets()`. We want the results of this conservation problem for each planning unit to be binary (i.e., 1/0, yes/no, TRUE/FALSE), which means that a whole planning unit (and not part of it) is selected for inclusion in a protected area. Thus, the algorithm will assign whether a full planning unit has been selected or not (using `add_binary_decisions()`). Note that you can add a fraction of a planning unit by using `add_proportion_decisions()` instead.
...
There were a lot of individual planning units selected in the solution. This can be problematic to include in an actual protected area.
...
The selected planning units are more aggregated, BUT this will lead to a more expensive solution.

### Penalty 2: Species penalty

> SHOULD WE ADD AT LEAST ONE OTHER PENALTY EXAMPLE?

There are several types of penalties, so it is worthwhile browsing through the `prioritizr` Reference tab for more informatiion: https://prioritizr.net/reference/index.html#penalties.
...
One example of constraints is to **lock in** specific areas, such as existing MPAs. This ensures that these MPAs will be part of the final solution.
...
There are several types of constraints, so it is worthwhile browsing through the `prioritizr` Reference tab for more information: https://prioritizr.net/reference/constraints.html.
...
We can compare the objective functions.
...
This objective in a conservation planning problem secures as much of the features as possible without exceeding a budget.
...
There are other objectives that can be used in the conservation problem - for more details, see https://prioritizr.net/reference/objectives.html. 
